...
public class BoundedListDataWordOracle implements DataWordOracle {
    public static final DataType<BigDecimal> INT_TYPE= new DataType<>("int", BuiltinTypes.DECIMAL);
    ...
    private boolean accepts(PSymbolInstance symInst, BoundedList list) {
        if (symInst.getBaseSymbol().equals(PUSH)) {
            list.push((BigDecimal) symInst.getParameterValues()[0].getValue());
            return true;
            ...
            list.insert((BigDecimal) symInst.getParameterValues()[0].getValue(),
                        (BigDecimal) symInst.getParameterValues()[1].getValue());
            return true;
        } else if (symInst.getBaseSymbol().equals(CONTAINS)) {
            return list.contains((BigDecimal) symInst.getParameterValues()[0].getValue());
        }
        return false;
    }
